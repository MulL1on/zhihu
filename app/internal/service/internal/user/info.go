...
func (s *SInfo) GetUserInfo(userBasic *user.Basic, userCounter *user.Counter, id any) error {
	sqlStr := "select digg_article_count,digg_shortmsg_count,followee_count,follower_count,got_digg_count,got_view_count,post_article_count,post_shortmsg_count,select_online_course_count from user_counter where id=?"
	...
	sqlStr = "select description,avatar,company,job_title from user_basic where id=?"
...
func (s *SInfo) UpdateUserInfo(userBasic *user.Basic, id any) error {
	sqlStr := "update user_basic set avatar=?,description=?,company=?,job_title=? where id=?"
