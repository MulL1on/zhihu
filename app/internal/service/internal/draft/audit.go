...
func (s *SAudit) CreateDraft(userId any, draft *draft.Draft) error {
	...
	sqlStr1 := "insert into draft (content, brief_content, title, category_id, user_id,create_time,update_time) VALUES (?,?,?,?,?,?,?)"
	res, err := tx.Exec(sqlStr1, draft.Content, draft.BriefContent, draft.Title, draft.CategoryId, userId, time.Now(), time.Now())
	if err != nil {
		tx.Rollback()
	...
	if len(draft.TagsIds) != 0 {
		sqlStr2 := "insert into  tag  (tag_id,context_id,id_type) values (?,?,?)"
		for _, v := range draft.TagsIds {
			_, err = tx.Exec(sqlStr2, v, draft.DraftId, "4")
			if err != nil {
				tx.Rollback()
...
func (s *SAudit) UpdateDraft(userId any, draft *draft.Draft) error {
	...
	sqlStr1 := "update draft set content=?,brief_content=?,title=?,category_id=? where draft_id=?&&draft.user_id=?"
	ret1, err := tx.Exec(sqlStr1, draft.Content, draft.BriefContent, draft.Title, draft.CategoryId, draft.DraftId, userId)
	if err != nil {
		tx.Rollback()
	...
	sqlStr2 := "delete from tag where context_id=?&id_type=?"
	_, err = tx.Exec(sqlStr2, draft.DraftId, "4")
	if err != nil {
		tx.Rollback()
	...
	sqlStr3 := "insert into  tag  (tag_id,context_id,id_type) values (?,?,?)"
	for _, v := range draft.TagsIds {
...
func (s *SAudit) GetDetail(draftId string, draft *draft.Draft) error {
	sqlStr := "select content, brief_content, title, category_id, draft_id, user_id, create_time, update_time from draft where draft_id=?"
	err := g.MysqlDB.QueryRow(sqlStr, draftId).Scan(&draft.Content, &draft.BriefContent, &draft.Title, &draft.CategoryId, &draft.DraftId, &draft.UserId, &draft.CreateTime, &draft.UpdateTime)
	if err != nil {
...
func (s *SAudit) DeleteDraft(draftId string) error {
	...
	sqlStr2 := "delete from tag where context_id=?&id_type=?"
	_, err = tx.Exec(sqlStr2, draftId, "4")
	if err != nil {
		tx.Rollback()
